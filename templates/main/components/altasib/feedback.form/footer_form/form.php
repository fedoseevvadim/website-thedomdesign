...
<?							if(!empty($_POST["FIELDS"][$arField["CODE"]])):?>
								<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=trim(htmlspecialcharsEx($_POST["FIELDS"][$arField["CODE"]]))?>" class="afbf_inputtext inputbox" readonly="readonly" onclick="BX.calendar({node:this,field:'FIELDS[<?=$arField["CODE"]?>]',form:'',bTime:<?=$bShowTime?>,currentTime:'<?=(time()+date("Z")+CTimeZone::GetOffset())?>',bHideTime:<?=$bHideTime?>});" />
<?							else:?>
								<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=$arField["DEFAULT_VALUE"]?>" class="afbf_inputtext inputbox" readonly="readonly" onclick="BX.calendar({node:this,field:'FIELDS[<?=$arField["CODE"]?>]',form:'',bTime:<?=$bShowTime?>,currentTime:'<?=(time()+date("Z")+CTimeZone::GetOffset())?>',bHideTime:<?=$bHideTime?>});" />
<?							endif;?>
...
?>
					<div class="afbf_inputtext_bg" id="error_<?=$arField["CODE"]?>">
<?						if(!empty($_POST["FIELDS"][$arField["CODE"]])):?>
							<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=trim(htmlspecialcharsEx($_POST["FIELDS"][$arField["CODE"]]))?>" class="afbf_inputtext inputbox" placeholder="<? echo $arField["NAME"]?>" />
<?						elseif(!empty($arField["AUTOCOMPLETE_VALUE"])):
							$readonly = "";
							...
							<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" <?=$readonly?>" class="afbf_inputtext inputbox" placeholder="<? echo $arField["NAME"]?>" />
<?						else:?>
							<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=$arField["DEFAULT_VALUE"]?>" class="afbf_inputtext inputbox" onblur="if(this.value==''){this.value='<?=$arField["DEFAULT_VALUE"]?>'}" onclick="if(this.value=='<?=$arField["DEFAULT_VALUE"]?>'){this.value=''}" placeholder="<? echo $arField["NAME"]?>" />
<?						endif;?>
...
<?					endif;?>
						<div class="afbf_input_group">
							<input type="text" size="40" id="<?=$arField["CODE"]?>1" name="FIELDS[myFile][<?=$arField["CODE"]?>]" value="" class="afbf_inputtext inputbox" />
...
<?						if($fVerComposite) $frame->end();?>
						...
						<div class="afbf_inputtext_bg"><input type="text" class="afbf_inputtext inputbox" id="captcha_word_<?=$ALX?>" name="captcha_word" size="30" maxlength="50" value=""></div>
...
<?		echo bitrix_sessid_post()?>
		<div class="afbf_submit_block">
			<input type="submit" class="fb_close afbf_btn" id="fb_close_<?=$ALX?>" name="SEND_FORM" value="Заказать консультацию" />
		</div>
	</form>
</div>
